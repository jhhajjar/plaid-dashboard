...
def apply_filters(
    transactions: List[TransactionEntity], start: str, end: str
    ...
    return list(
        filter(
            lambda x: x.get_date() <= end_datetime and x.get_date() >= start_datetime,
            transactions,
        )
    )
...
def apply_updates(
    transactions: List[TransactionEntity], updates: List[TransactionEntity]
) -> List[TransactionEntity]:
    to_delete = [
        {"transaction_id": tr.id, "account_id": tr.account_id} for tr in updates
    ]
    return apply_additions(apply_deletions(transactions, to_delete), updates)


def apply_deletions(
    transactions: List[TransactionEntity], deletions: List[dict]
) -> List[TransactionEntity]:
    keys_to_delete = [f"{tr['transaction_id']}{tr['account_id']}" for tr in deletions]
...
def validate(tr: TransactionEntity) -> None:
    ...
    if tr.amount != transform_amount(tr):
        raise ValueError(
            f"Transaction {tr.id} has amount {tr.amount} not equal to transformed plaid amount {transform_amount(tr)}"
        )
...
def map_transaction_to_dto(transaction: TransactionEntity) -> TransactionDTO:
    ...
    return TransactionDTO(date, transaction_id, merchant_name, category, amount)
